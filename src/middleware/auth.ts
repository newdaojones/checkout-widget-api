...
export const authMiddlewareForGraphql: RequestHandler = async (
  req,
  res,
  next
) => {
...
export const authMiddlewareForPartner: RequestHandler = async (
  req,
  res,
  next
) => {
  ...
  jwt.verify(token, Config.jwtSecret, async (err: any, decoded: any) => {
    ...
    const partner = await user.getPartner();

    if (!partner) {
      res.status(403).json({
        message: "User not associated with partner",
      });
      return;
    }
