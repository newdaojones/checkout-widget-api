...
router.post("/partners", async (req, res) => {
  ...
  try {
    ...
    const existingUserEmail = await KycLink.findOne({
      where: {
        email: data.email,
      },
    });
    ...
    const partner = await Partner.sequelize.transaction(async (t) => {
      ...
      const res = await bridgeService.createKycLink(
        user.id,
      ...
      await KycLink.create({
        ...
        email: res.email,
        type: res.type,
        kycLink: res.kyc_link,
        tosLink: res.tos_link,
        kycStatus: res.kyc_status,
        tosStatus: res.tos_status,
      });
...
router.post("/partners/login", async (req, res) => {
  try {
    ...
    const partner = await User.findUser(email, password);
